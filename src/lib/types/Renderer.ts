...
import {GridPosition} from "$lib/types/Graphics";
...
export class Renderer {
    ...
    public drawBackground() {
        ...
        if (!this.floorPlanImage) {
            let img = this.floorPlanImage ?? new Image();
            img.onload = () => {
    ...
    public colorRoom(room: Room) {
        ...
        this.ctx.fillStyle = grd && room.quality === 0 ? "rgba(10, 150, 255, .5)" : grd;
        this.ctx.beginPath()
